...
share_dir = r'\\192.168.60.72\Test\TestData(72)\166024245\data_spp_replay'
...
excluded_case = ['FILE068', 'FILE069', 'FILE070', 'FILE071', 'FILE076', 'FILE077', 'FILE078', 'FILE079', 'FILE080',
                     'FILE083', 'FILE084', 'FILE085', 'FILE086', 'FILE087', 'FILE099']
local_existed_cases = []
utc = 0
tunnels_list = []
dir_names = []
...
if __name__ == '__main__':
     ...
     # 是否清空结果文件夹。若清空，则强制开启if_copy
     if if_copy is True:
          if_confirmed = input(f"Result directory: {local_results_dir}\nClear analyze result directory?")
          if if_confirmed == '' or if_confirmed == 'y' or if_confirmed == 'yes':
               print('Clear all...')
               shutil.rmtree(local_results_dir)
               while True:
                    if os.path.exists(local_results_dir) is False:
                         os.mkdir(local_results_dir)
                         break
               if_copy = True  # 强制开启copy
          else:
               print('Do not clear.')
     else:
          pass
