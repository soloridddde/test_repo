...
share_dir_path = r'\\192.168.60.72\Test\TestData(72)\166023106\data_spp_replay'
...
excluded_case = ['FILE068', 'FILE069', 'FILE070', 'FILE071', 'FILE076', 'FILE077', 'FILE078', 'FILE079', 'FILE080',
				 'FILE083', 'FILE084', 'FILE085', 'FILE086', 'FILE087', 'FILE105']
...
def find_tunnel(infile, outfile):
	...
	data_lost_list = []
	f_in = open(infile, encoding='UTF-8')
	f_out = open(outfile[2], mode='w', encoding='utf-8')
	...
	for tunnel in tunnels_list:
		...
		for line in kml_lines:
			if 'UTC时间' not in line:
				...
				if tn_start_time not in line:
					continue
				else:
					cur_line_index = kml_lines.index(line)
					scene_line_index = cur_line_index + 5
